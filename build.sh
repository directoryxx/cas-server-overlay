...
function gencert() {
	...
	DNAME="${DNAME:-CN=server.riset.com,OU=Riset,OU=Com,C=US}"
	CERT_SUBJ_ALT_NAMES="${CERT_SUBJ_ALT_NAMES:-dns:server.riset.com,ip:172.16.252.159,dns:server.local.com}"
	...
	keytool -exportcert -alias cas -storepass changeit -keystore /etc/cas/thekeystore -file /etc/cas/certificate.der
}
